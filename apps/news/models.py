...
class CommentModel(BaseModel):
    ...
    parents = models.ForeignKey('self', on_delete=models.CASCADE)  # 自关联，用于创建自评论
...
class NewsHots(BaseModel):
    ...
    priority = models.IntegerField(verbose_name='新闻优先级', help_text='新闻优先级',default=3)
...
class NewsBanner(BaseModel):
    ...
    priority = models.IntegerField(verbose_name='轮播图片优先级', help_text='轮播图片优先级',default=6)  # 优先级默认为第六级
